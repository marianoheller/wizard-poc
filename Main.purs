...
app :: Component Unit
app = unit # make (createComponent "WizardExample") { initialState, render }
  where
  initialState =
  ...
  send self = case _ of
    OnChange modify -> self.setState _ { formData = modify self.state.formData }
    SetModified -> case W.stepIdentifier self.state.wizardStep of
      ...
      Nothing -> pure unit
    SetWizardStep wizardStep -> self.setState _ { wizardStep = wizardStep }
    Finalize result -> self.setState _ { result = Just result }
